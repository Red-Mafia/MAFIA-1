...
# coding=utf-8
# Coded By RED-MAFIA
# My Facebook Page ( RED-MAFIA)

#      (C) Copyright 407 Authentic Exploit
#      Rebuild Copyright Can't make u real programmer:)
#      Coded By RED-MAFIA(RK).
...
logo = ''' \033[0;96m   ___    ___    ____   
 \033[0;96m  / \x1b[1;93m® \033[0m| Created By RED-MAFIA(RK)
 \033[1;93m /  \033[0m| Github.com/Red-Mafia
 \033[1;93m/ \033[0;91mv2.0 \033[0m| page.https://www.facebook.com/ᎡᎬᎠ-ᎷᎪҒᏆᎪ-102896575436747/
...
def yayanxd():
    ...
    try:
        ...
        print '%s║%s '%(O,N)
        print '%s╠══[%s Welcome%s%s%s Ngentod \033[0;96m]'%(O,N,H,nama,N);time.sleep(2)
        print '%s╠══[%s Before Using This Script Pleasek \033[0;96m]'%(O,N);time.sleep(2)
        print '%s╠══[%s FOLLOW MY FB PAGE RED-MAFIA Ya:v \033[0;96m]'%(O,N);time.sleep(2)
        open('.memek.txt', 'w').write(kontol)
        raw_input('\n%s[•]%s Press Enter '%(O,N));wuhan(kontol)
        os.system('xdg-https://www.facebook.com/ᎡᎬᎠ-ᎷᎪҒᏆᎪ-102896575436747/')
        moch_yayan()
    except KeyError:
...
def moch_yayan():
    ...
    if pepek == '':
        ...
        try:
            ...
            print("\n%s[%s!%s] opshh you are not getting resultsl"%(N,K,N))
...
def info_tools():
    ...
    print '%s[%s>%s] Fb page      : Red-Mafia (Handsome People Hehe)'%(N,H,N);time.sleep(0.09)
    print '%s[%s>%s] Author   : Red-Mafia(RK).'%(N,H,N);time.sleep(0.09)
    print '%s[%s>%s] Github   : https://github.com/Red-Mafia'%(N,H,N);time.sleep(0.09)
    print '%s[%s>%s] Facebook : https://www.facebook.com/ᎡᎬᎠ-ᎷᎪҒᏆᎪ-102896575436747/'%(N,H,N);time.sleep(0.09)
    print '%s[%s>%s] watsapp: https://wa.me/+923188214452'%(N,H,N);time.sleep(0.09)
    print '%s[%s#%s]'%(N,O,N), 52 * '\x1b[1;96m-\x1b[0m';time.sleep(0.09)
    ...
    print '%s[%s>%s] • Ali Raza'%(N,H,N);time.sleep(0.09)
...
def seting_yntkts():
    ...
    ytbjts = raw_input('\n%s[%s?%s] menu : '%(N,O,N))
    if ytbjts == '':
        print '\n%s[%s×%s] Fill Empty Places Kentod'%(N,M,N);time.sleep(2);seting_yntkts()
    elif ytbjts =='1':
...
def yo_ndak_tau_ko_tanya_saia():
    ...
    if anjng == '':
        print '\n%s[%s×%s] Fill empty places Kentod'%(N,M,N);yo_ndak_tau_ko_tanya_saia()
    try:
...
class __crack__:
            ...
            while True:
                ...
                if pwek == '':
                    ...
                    def __yan__(ysc=None): # ycs => Ali raza:3
                        ...
                        print '%s║%s '%(O,N)
                        if cin == '':
                            print '%s[%s×%s] Fill Empty Place kentod'%(N,M,N);self.__yan__()
                            print '%s║%s '%(O,N)
                        elif cin == '1':
    ...
    def __pler__(self):
        ...
        print '%s║%s '%(O,N)
        if yan == '':
            print '%s[%s×%s] Fill Empty places kentod'%(N,M,N);self.__pler__()
            print '%s║%s '%(O,N)
        elif yan in ('1', '01'):
            print '\033[0;96m╠══[%s•%s\033[0;96m] \033[0;97mAccount OK Saved In > Results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\033[0;96m╠══[%s•%s\033[0;96m] \033[0;97mAccount CP Saved In > Results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\033[0;96m╚══[%s!%s\033[0;96m] \033[0;97mTurn on/off airplane mode for 5 sec\n'%(H,N)
            with YayanGanteng(max_workers=30) as (__Red-Mafia__):
